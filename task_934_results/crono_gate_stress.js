 ...
 *
 * Uso: node crono_gate_stress.js [--duration=600] [--seeds=5]
 */
'use strict';
const fs = require('fs');
...
const SEEDS = [12345].slice(0, OPTS.seeds);
...
function runSeed(seed, cronoOn) {
  ...
  // Estrés: régimen de luz escalonado idéntico OFF/ON (determinista)
  const lightAt = (t) => (t < 120 ? 0.3 : t < 300 ? 3.0 : t < 480 ? 0.5 : 1.0);
  while (api.sim.time < OPTS.durationSec) {
    api.sim.solarEnergy = lightAt(api.sim.time);
    api.compactIfNeeded();
    api.rebuildGrid();
