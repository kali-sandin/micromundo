#!/bin/bash
# task_916 Gate 1: 5 seeds x 10m shadow prey-stamina (protocolo Jared)
set -e
cd "$(dirname "$0")"
mkdir -p task_916_results
SEEDS=(12345 20264 28183 36102 44021)
LOG=task_916_results/batch.log
echo "=== Batch 5x10m shadow prey-stamina started $(date -Iseconds) ===" | tee "$LOG"
for i in "${!SEEDS[@]}"; do
  seed="${SEEDS[$i]}"; idx=$((i+1))
  out="task_916_results/run_sh_${idx}_seed${seed}.json"
...
node analyze-task916.js run_sh | tee task_916_results/gate1_analysis.txt
